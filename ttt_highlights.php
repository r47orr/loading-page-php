...
<html lang="pt-br">
    <?php
        ...
        $steamids = [];
        $steamids = $config["users"]["highlights"]["items"];

        // ------------------------------------------------- 

        $staffids = [];
        $staffids = $config["users"]["staff"]["items"];

        // HIGHLIGHTED PLAYERS' PROFILE INFO REQUESTS 
        $str_steamids = '';
        foreach($steamids as $str_steamid) {
            $str_steamids .= $str_steamid.',';
        }
        ...
        // STAFF MEMBERS' PROFILE INFO REQUESTS
        $str_staffids = '';
        foreach($staffids as $staffid) {
            $str_staffids .= $staffid.',';
        }

        // realizar requisição das informações dos perfis públicos dos jogadores necessários à API do Steam
        @ $url = file_get_contents("http://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=".API_KEY."&steamids=".$str_staffids);
            $content = json_decode($url, true);
            $staffdata = [];
            $c = count($staffids);
            for($i = 0; $i < $c; $i++) {
                $staffdata[$content['response']['players'][$i]['steamid']] = $content['response']['players'][$i];
            }
    ?>
    <head>
        ...
        <div class="playerinfo">
            <div class="content">
                ...
                <div class="highlights">
                    <h3><?php echo $config["users"]["highlights"]["title"]; ?></h3>
                    <ul class="highlights">
                        <?php
                            $c = count($steamids);
                            for ($i = 0; $i < $c; $i++) {
                                ...
                                echo "<li class='specialply'>\n";
                                echo $i+1 . "° colocado: ". '<img src="'.$img.'"></img>' . " $name";
                                echo "</li>\n";
                            }
                        ?>
                    </ul>
                ...
                <ul>
                    <li><h2>Backstabber Brasil</h2></li>
            ...
            <div class="content">
                ...
                <ul class="staffmembers">
                    <?php
                        $c = count($staffids);
                        for ($i = 0; $i < $c; $i++) {
                            $name = $staffdata[$staffids[$i]]['personaname'];
                            $img = $staffdata[$staffids[$i]]['avatar'];
                            echo "<li>\n";
